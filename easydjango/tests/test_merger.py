...
class TestSettingMerger(TestCase):
    ...
    def test_priority(self):
        merger = SettingMerger('test', None, [DictProvider({'X': 1}), DictProvider({'X': 2})])
        merger.process()
    ...
    def test_parse(self):
        merger = SettingMerger('test', None, [DictProvider({'X': 1, 'Y': 'x{X}'}), DictProvider({'X': 2})])
        merger.process()
        self.assertEqual({'X': 2, 'Y': 'x2', 'PROJECT_NAME': 'test'}, merger.settings)

    def test_loop(self):
        merger = SettingMerger('test', None, [DictProvider({'X': '{Y}', 'Y': '{Z}', 'Z': '{X}'})])
